 ...
 * @param lang - Output language ('chinese', 'english', etc.) for UI chrome strings.
 */
export async function generateHTMLViewer(wikiDir: string, projectName: string, lang?: string): Promise<string> {
  // Load module tree
  ...
  const html = buildHTML(projectName, moduleTree, pages, meta, lang);
...
function t(lang: string | undefined, en: string, zh: string): string {
  ...
  if (l === 'chinese' || l.startsWith('zh')) return zh;
  return en;
}
...
function buildHTML(
  ...
  parts.push('<html lang="' + t(lang, 'en', 'zh') + '">');
  parts.push('<head>');
  parts.push('<meta charset="UTF-8">');
  parts.push('<meta name="viewport" content="width=device-width, initial-scale=1.0">');
  parts.push('<title>' + esc(projectName) + ' — ' + t(lang, 'Wiki', '维基') + '</title>');
  ...
  parts.push('<body>');
  parts.push(
    '<button class="menu-toggle" id="menu-toggle" aria-label="' + t(lang, 'Toggle menu', '切换菜单') + '">&#9776;</button>',
  );
  ...
  parts.push('<div class="sidebar-footer">' + t(lang, 'Generated by GitNexus', '由 GitNexus 生成') + '</div>');
  parts.push('</nav>');

  // Content
  parts.push('<main class="content" id="content">');
  parts.push('<div class="empty-state"><h2>' + t(lang, 'Loading…', '加载中…') + '</h2></div>');
  parts.push('</main>');
  parts.push('</div>');
  ...
  parts.push('    <input type="text" name="q" placeholder="' + t(lang, 'Ask anything about this codebase...', '询问关于此代码库的任何问题...') + '" autocomplete="off">');
  parts.push('    <button type="submit">' + t(lang, 'Ask', '提问') + '</button>');
  parts.push('  </form>');
  parts.push('</div>');

  // ── Script ──
  parts.push('<script>');
  parts.push('var NAV_OVERVIEW = ' + JSON.stringify(t(lang, 'Overview', '概览')) + ';');
  parts.push('var NAV_MODULES = ' + JSON.stringify(t(lang, 'Modules', '模块')) + ';');
  parts.push('var MSG_LOADING = ' + JSON.stringify(t(lang, 'Loading…', '加载中…')) + ';');
  parts.push('var MSG_NOT_FOUND = ' + JSON.stringify(t(lang, 'Page not found', '页面未找到')) + ';');
  parts.push('var MSG_NOT_FOUND_DESC = ' + JSON.stringify(t(lang, '.md does not exist.', '.md 文件不存在。')) + ';');
...
(function() {
  ...
  function renderNav() {
    ...
    html += '<a class="nav-item overview" data-page="overview" href="#overview">' + NAV_OVERVIEW + '</a>';
    html += '</div>';
    if (TREE.length > 0) {
      html += '<div class="nav-group-label">' + NAV_MODULES + '</div>';
  ...
  function navigateTo(page) {
    ...
    if (!md) {
      contentEl.innerHTML = '<div class="empty-state"><h2>' + MSG_NOT_FOUND + '</h2><p>' + escH(page) + MSG_NOT_FOUND_DESC + '</p></div>';
      return;
    }
